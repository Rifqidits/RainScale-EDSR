...
def selectPrecipitation(dataset, specificDate):
    # Ensure time axis is in datetime64 format
    time_values = dataset.time.values.astype('datetime64[ns]')

    # Ensure specificDate is also in datetime64[ns]
    specificDate = np.datetime64(specificDate, 'ns')
    ...
    # Range check
    if specificDate < min_time or specificDate > max_time:
        raise ValueError(f"Date {specificDate} is outside the dataset range: {min_time} to {max_time}")

    # Proceed to find the closest time index
    time_values_numeric = time_values.astype(np.int64)
    specific_date_numeric = specificDate.astype(np.int64)
